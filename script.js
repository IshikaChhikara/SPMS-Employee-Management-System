...
function showAdminVacDetail(i) {
  ...
  const referrals = LS.get('spms_referrals') || [];
  const vac = vacs[i];
  if (!vac) { toast('Vacancy not found', 'error'); return; }
  
  const applicants = referrals.filter(r => r.vacId === vac.id);
  const referred = applicants.filter(r => r.referredBy && r.referredBy !== 'null');
  const external = applicants.filter(r => !r.referredBy || r.referredBy === 'null');
...
function saveVacancy() {
  ...
  resetVacForm();
  toggleVacView();
  setTimeout(() => renderVacancies(), 100);
}
...
function renderProfile() {
  ...
  const emp = emps.find(e => e.id === currentUser.empId);
  if (!emp) return;
  const photoHtml = emp.photo ? `<img src="${emp.photo}" alt="Photo">` : `<span style="font-size:1.8rem">${emp.name[0]}</span>`;

  document.getElementById('idCardWrap').innerHTML = `
    <div class="id-card">
      <div class="id-card-logo">SPMS Corp</div>
      <div class="id-card-photo">${photoHtml}</div>
      <div class="id-card-name">${emp.name}</div>
      <div class="id-card-role">${emp.pos}</div>
      <div class="id-card-divider"></div>
      <div class="id-card-row"><span class="id-card-key">Employee ID</span><span class="id-card-val">${emp.id}</span></div>
      <div class="id-card-row"><span class="id-card-key">Department</span><span class="id-card-val">${emp.dept}</span></div>
      <div class="id-card-row"><span class="id-card-key">Joined</span><span class="id-card-val">${fmtDate(emp.joinDate)}</span></div>
      <div class="id-card-row"><span class="id-card-key">Email</span><span class="id-card-val" style="word-break:break-all;font-size:.65rem">${emp.email}</span></div>
      <div class="id-card-row"><span class="id-card-key">Phone</span><span class="id-card-val">+91 ${emp.phone}</span></div>
      <div class="id-card-footer">${emp.id} • ${emp.dept.toUpperCase().slice(0,3)}</div>
    </div>
  `;
